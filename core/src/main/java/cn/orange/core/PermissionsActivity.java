...
public abstract class PermissionsActivity extends AppCompatActivity
    ...
    private boolean checkPermissions(@NonNull String[] permissions) {
        List<String> needRequestPermissions = new ArrayList<>(1);
     ...
     */
    private void handlePermissionsResult(@NonNull final String[] permissions,
                                         @NonNull final int[] grantResults) {
        List<String> requestFailurePermissionsList = new ArrayList<>(1);
        for (int index = 0; index < permissions.length; index++) {
            String permission = permissions[index];
            boolean isPermissionGranted = grantResults[index] == PackageManager.PERMISSION_GRANTED;
            Log.d(TAG, "RequestPermission:" + permission + " granted:" + isPermissionGranted);
            if (!isPermissionGranted) {
                requestFailurePermissionsList.add(permission);
            }
        }
        if (!requestFailurePermissionsList.isEmpty()) {
